...
main(int ac, char *av[]) {
    ...
    /* Adjust the score if the IP has sent from multiple addresses */
    key = yaslcatprintf(yaslauto(prefix), ":ip:%s:rfc5321.mailfrom", ip);
    ...
    key = yaslcatprintf(yaslauto(prefix), ":ip:%s", ip);
